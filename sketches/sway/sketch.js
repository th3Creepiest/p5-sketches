 ...
 *
 * A smoky, colorful tangle.
 * Each particle is assigned its own flow field to move it around.
 * Each particle-pair will draw with a color chosen out of a palette.
 *
 ******************************************************************************/
...
function setup() {
  ...
  // explicitly initialize empty arrays here (for when setup() is called on mouse click)
  particles = []
  simplex = []
  flowfield = []
...
linkup = function () {
  ...
  for (let i = 0; i < particles.length; i++) {
    for (let j = 0; j < i; j++) {
      // Draw from jth element up to i. Prevents double lines or drawing line from self to self
      ...
      if (d <= dmax) {
        // save some cycles - don't bother doing calculations if distance is too great anyway.
        d = constrain(d, 20, dmax)
        let alpha = map(d, 0, dmax, alphaMax, 0, 1)
        let palletteIndex = (i + j) % colours.length // select a color for a pair of particles
...
setPalette = function () {
  // Forest
  palette.push([color(109, 64, 13), color(3, 137, 113), color(59, 158, 184), color(113, 193, 108), color(52, 87, 32)])

  // Warm
  palette.push([color(211, 217, 35), color(214, 217, 143), color(241, 242, 206), color(140, 91, 48), color(64, 57, 26)])
  ...
  // Soda
  palette.push([color(2, 7, 114), color(131, 191, 3), color(242, 202, 4), color(242, 127, 26), color(242, 101, 19)])
